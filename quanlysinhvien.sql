...
select studentname,sub.subname,m.mark from student join mark m on student.studentid = m.studentid join  subject sub on m.subid = sub.subid group by m.mark order by m.mark desc;
